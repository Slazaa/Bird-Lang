...
pub static VAR_DECL_PATTERNS: [(&str, &str, PatternFunc<Node>); 4] = [
...
fn var_decl(nodes: &[Node]) -> Result<Node, String> {
	...
	Ok(Node::VarDecl(VarDecl { id, var_type: None, val: None }))
}
...
fn var_decl_expr(nodes: &[Node]) -> Result<Node, String> {
	...
	Ok(Node::VarDecl(VarDecl { id, var_type: None, val }))
}
...
fn var_decl_typed(nodes: &[Node]) -> Result<Node, String> {
	...
	Ok(Node::VarDecl(VarDecl { id, var_type: Some(var_type), val: None }))
}
...
fn var_decl_typed_expr(nodes: &[Node]) -> Result<Node, String> {
	...
	Ok(Node::VarDecl(VarDecl { id, var_type: Some(var_type), val }))
}