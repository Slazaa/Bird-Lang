...
impl Compiler {
	...
	fn eval(&mut self, node: &Node) -> Result<String, Feedback> {
		match node {
			...
			Node::FuncProto { identifier, params, return_type, .. } => self.func_proto(identifier, params, return_type),

			Node::FuncItem { proto, body } => self.func_item(proto, body),
			Node::VarItem { identifier, var_type, value, .. } => self.var_item(identifier, var_type, value),
			Node::TypeItem { identifier, value, .. } => self.type_item(identifier, value),
			...
			Node::TypePtr { hodling_type, mutable } => self.type_ptr_node(hodling_type, *mutable)
		}
	}
	...
	fn func_proto(&mut self, identifier: &Node, params: &Vec<Node>, return_type: &Node) -> Result<String, Feedback> {
		let mut res = String::new();

		write!(&mut res, "{} {}(", self.eval(return_type)?, self.eval(identifier)?).unwrap();
	...
	fn var_item(&mut self, identifier: &Node, var_type: &Node, value: &Option<Node>) -> Result<String, Feedback> {
		let mut res = String::new();
	...
	fn type_item(&mut self, identifier: &Node, value: &Node) -> Result<String, Feedback> {
		Ok(format!("typedef {} {};", self.eval(value)?, self.eval(identifier)?))
	}
	...
	fn if_statement(&mut self, condition: &Node, body: &Node) -> Result<String, Feedback> {
		Ok(format!("if({}){{{}}}", self.eval(condition)?, self.eval(body)?))
	}

	fn loop_statement(&mut self, condition: &Node, body: &Node) -> Result<String, Feedback> {
		Ok(format!("while({}){{{}}}", self.eval(condition)?, self.eval(body)?))
	}
	...
	fn type_ptr_node(&mut self, hodling_type: &Node, mutable: bool) -> Result<String, Feedback> {
		match mutable {
			true => Ok(format!("{}*", self.eval(hodling_type)?)),
			false => Ok(format!("const {}*", self.eval(hodling_type)?))
		}
	}
}
