...
impl Parser {
        ...
        match self.parent_node() {
            Node::Program { .. } => match self.current_token().token_type() {
                TokenType::Keyword => matches!(
                    ...
                    "const" | "enum" | "func" | "struct" | "type" | "var"
                ),
                _ => false,
            },
            ...
            match self.current_token().symbol() {
                ...
                _ => {
                    return Err(Error::unexpected(
                        next_pub.pos(),
                        &format!("'{}'", self.current_token().symbol()),
                    ))
                }
            }
        }
    ...
    fn func_item(&mut self) -> Result<Node, Feedback> {
        ...
        let mut func_proto = Node::FuncProto {
            public,
            identifier: Box::new(identifier),
            params,
            return_type: Box::new(Node::identifier("void", self.current_token().pos())),
        };
        ...
        let mut func_item = Node::FuncItem {
            proto: Box::new(func_proto.clone()),
    ...
    fn struct_item(&mut self) -> Result<Node, Feedback> {
        ...
        let mut struct_item = Node::StructItem {
            public,
            identifier: Box::new(identifier),
            fields: vec![],
        };
    ...
    fn if_stmt(&mut self) -> Result<Node, Feedback> {
        let mut node = Node::IfStmt {
            condition: Box::new(Node::Undefined),
            body: Box::new(Node::block(vec![])),
        };
    ...
    fn type_node(&mut self) -> Result<Node, Feedback> {
        match self.current_token().token_type() {
            TokenType::Identifier => Ok(Node::Type {
                identifier: Box::new(Node::identifier(
                    self.current_token().symbol(),
                    self.current_token().pos(),
                )),
            }),
